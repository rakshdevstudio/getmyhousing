...
public class LoginServiceImpl implements LoginService {
	...
	public List<UserRole> getAllUserRoles(Long userId) {
		...
		StringBuffer sqlQuery = new StringBuffer("from UserRole r where r.userId =:userId");
	...
	public boolean isUserAccessible(Long userId, List<String> roles) {
		...
		StringBuffer sqlQuery = new StringBuffer("from UserRole r where r.userId =:userId AND r.role IN (:roles)");
		Query query = entityManager.createQuery(sqlQuery.toString());
		query.setParameter("userId", userId);
